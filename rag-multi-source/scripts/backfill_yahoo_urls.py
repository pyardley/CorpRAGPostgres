...
def main() -> int:
    ...
    # Match every URL flavour earlier ingestor revisions ever wrote.
    # The trickiest case is the most recent: rows whose URL already
    # starts with /n/search/keyword=... but were generated by the
    # single-encoded / no-before: revision. We detect those by the
    # absence of the double-encoded ``before:`` token (``%2520before``
    # after Yahoo's path-decode wrap, equivalent to ``%2520before%253A``
    # in the URL string) — anything emitted by the current builder
    # always contains it, so the negation reliably picks out stale rows
    # without re-migrating the already-correct ones.
    select_sql = text(
        """
        SELECT id,
               url,
