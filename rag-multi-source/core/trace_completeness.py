...
def _candidate_names(hits: list[RetrievedChunk]) -> set[str]:
    ...
    narrow, favoring silence over noise.

    Retrieval for a tracing question routinely pulls in objects that are
    only loosely related (shared vocabulary, not an actual dependency) —
    e.g. a sibling report procedure that happens to mention "segment" or
    "region" too. Treating every retrieved object's name and every temp
    table across ALL hits as "should have been mentioned" produces exactly
    that false-positive noise (confirmed against a real run: an unrelated
    procedure's own internal temp tables, and its own name, both got
    flagged as "missing" from an answer that correctly ignored them).

    So: only the single highest-scored SQL hit (`hits` arrives pre-sorted
    by score, descending — from `core.retriever.retrieve` regardless of
    whether reranking ran) is treated as the object actually being
    explained. Its own internal temp tables are always fair candidates.
    Another retrieved object's name only counts if the anchor's own text
    literally references it — i.e. it's a real dependency the anchor
    calls/joins/writes to, not just something that happened to retrieve
    alongside it. The anchor's own name is never a candidate against
    itself; not restating "I am now explaining X" isn't an omission.
    """
    sql_hits = [
        h
        for h in hits
